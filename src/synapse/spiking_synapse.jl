...
@with_kw mutable struct SpikingSynapse
    ...
    tpre::Vector{SNNFloat} = zeros(W) # presynaptic spiking time
    tpost::Vector{SNNFloat} = zeros(W) # postsynaptic spiking time
    Apre::Vector{SNNFloat} = zeros(W) # presynaptic trace
    Apost::Vector{SNNFloat} = zeros(W) # postsynaptic trace
...
#=@replace=# function plasticity!(c::SpikingSynapse, param::SpikingSynapseParameter, dt::SNNFloat, t::SNNFloat)
    @inbounds for j in 1:(length(colptr) - 1)
        if fireJ[j]
            for s in colptr[j]:(colptr[j+1] - 1)
